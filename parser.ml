...
(* SCANNER. Lexical scanner for Lisp. It also ignores comments. *)
...
module type Parsers =
sig
  val makeParser: string -> unit -> thing (* Only visible calls are in signature*)
...
struct
  ...
  let makeParser path = 
   let nextToken = (Scanner.makeScanner path) in (*nextToken becomes function to be called later*)
      let token = ref (nextToken ()) in  (*token contains actual thing to be read*)
        (fun () -> (*makeParser initializes function that can be bound using let, and then called later*)
          let rec nextThing () = 
            match !token
            with  Scanner.CloseParenToken -> raise (Can'tParse "error") |
                  ...
                  Scanner.OpenParenToken -> token := nextToken (); nextThings () | (*Begins a list*)
          ...
          and nextThings () =
            ...
            with CloseParenToken -> token := nextToken (); Nil |
                 ...
                 thing -> let a = nextThing () in (*thing right after open parens*)
                          let b = nextThings () in (*everything until parens are closed*)
                          Cons(a, b) (*Cons cannot call funcs directly because order not guaranteed*)
          in
          nextThing () (*initial call, call inside variable token positioned cursor properly*)
        )
  ;;
end
;;